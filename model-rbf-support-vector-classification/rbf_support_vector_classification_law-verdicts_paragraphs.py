...
# 载入已经训练好的分类器
clf = joblib.load("..\\models-machine-learning\\rbf_support_vector_classification_law-verdicts_paragraphs.pkl")
...
for pages_folder in paragraphs_folder_name_list:
    print(pages_folder)
    paragraphs_text_file_name_list = listdir(folder_path_lines + "\\" + pages_folder)

    for file_name in paragraphs_text_file_name_list:
        with open(folder_path_lines + "\\" + pages_folder + "\\" + file_name, encoding="utf-8") as text_file_object:
            if text_file_object:
                ...
                # 打印预测结果
                if datasets_train.target_names[predicted[0]] == "target-paragraphs-law-verdicts":
